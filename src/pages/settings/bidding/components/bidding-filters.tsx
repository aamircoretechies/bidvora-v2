      ...
      <CardContent className="p-5 grid gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="grid gap-1.5">
            ...
            <div className="flex items-center gap-2">
              <Input type="number" min={0} value={data?.minBudget ?? ''} onChange={(e) => onChange?.('minBudget', parseInt(e.target.value) || 0)} className="text-sm" />
              <span className="text-secondary-foreground text-sm shrink-0">–</span>
              <Input type="number" min={0} value={data?.maxBudget ?? ''} onChange={(e) => onChange?.('maxBudget', parseInt(e.target.value) || 0)} className="text-sm" />
            </div>
          </div>
          <div className="grid gap-1.5">
            <Label className="text-sm font-semibold text-foreground">Hourly Rate Range</Label>
            <div className="flex items-center gap-2">
              <Input type="number" min={0} value={data?.minHourlyRate ?? ''} onChange={(e) => onChange?.('minHourlyRate', parseInt(e.target.value) || 0)} className="text-sm" />
              <span className="text-secondary-foreground text-sm shrink-0">–</span>
              <Input type="number" min={0} value={data?.maxHourlyRate ?? ''} onChange={(e) => onChange?.('maxHourlyRate', parseInt(e.target.value) || 0)} className="text-sm" />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="grid gap-1.5">
            ...
            <Input type="number" min={0} value={data?.bidFactorPercent ?? ''} onChange={(e) => onChange?.('bidFactorPercent', parseInt(e.target.value) || 0)} className="text-sm" />
          </div>
          <div className="grid gap-1.5">
            ...
            <Input type="number" min={0} value={data?.hourlyPrice ?? ''} onChange={(e) => onChange?.('hourlyPrice', parseInt(e.target.value) || 0)} className="text-sm" />
          </div>
        </div>
